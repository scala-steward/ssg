...
object FilePathPlatform {
  ...
  /** Pure-string rendering matching java.nio.file.Paths.get(path).toString: collapse duplicate separators and drop a trailing separator (the root "/" keeps its single separator), preserving "." /
    ...
    *
    * A native Windows drive-absolute input ("C:\\a" or "C:/a", no leading slash) is first lifted into the model form "/C:/a" (ISS-1384, porting the ISS-1383 JVM fix to JS parity): its backslash
    * separators become forward slashes and a leading '/' is prefixed so the path is POSIX-absolute, matching the "/C:/x" form the JVM and Native platforms produce. A bare colon or bare backslash
