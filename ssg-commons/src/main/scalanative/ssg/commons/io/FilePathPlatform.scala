...
object FilePathPlatform {
  ...
  /** Pure-string rendering matching java.nio.file.Paths.get(path).toString: collapse duplicate separators and drop a trailing separator (the root "/" keeps its single separator), preserving "." /
    ...
    *
    * A native Windows drive-absolute input ("C:\\a" or "C:/a", no leading slash) is first lifted into the model form "/C:/a" (ISS-1384, porting the ISS-1383 JVM fix to Native parity): its backslash
    * separators become forward slashes and a leading '/' is prefixed so the path is POSIX-absolute, matching the "/C:/x" form that cwd and fromNioPath already produce. A bare colon or bare backslash
    * (ISS-1128: "a:b", "a\\b") is NOT a drive-absolute form and is preserved unchanged.
    */
  ...
  /** True if `p` is a native Windows drive-absolute input ("C:\\..." or "C:/...", with NO leading slash): a single drive letter, then ':' at index 1, then a '/' or '\\' separator at index 2. This is
    * the raw form java.nio.Paths.get parses as absolute on Windows (Paths.get("C:\\a").isAbsolute == true); FilePath.of must lift it into the model's "/C:/..." form (ISS-1384, porting ISS-1383).
    * Requiring the separator at index 2 keeps a bare-colon filename ("a:b") and a drive-relative input ("C:a") out (ISS-1128 parity), and the leading letter+':' never matches a POSIX path nor the
    * already-lifted "/C:/x" model form.
    */
