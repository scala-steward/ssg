...
/** ISS-1384 [R0610]: cross-platform parity follow-up to ISS-1383 — the JS FilePathPlatform.of/renderPath must lift a native Windows drive-absolute input ("C:\\a" or "C:/a", with NO leading slash)
  ...
  *
  * This is a MODEL-LEVEL string defect, reproducible on ANY OS: the JS FilePathPlatform.isAbsolute only checks startsWith("/"), so FilePath.of("C:\\a").isAbsolute is false everywhere, and .toAbsolute
  * then string-joins the process cwd onto a path that is already drive-absolute. ISS-1383 fixed this on the JVM only; the JS (and Native) platform files never received the lift — the parity gap
  * ISS-1383's auditor flagged as ISS-1384.
  *
  ...
  * FilePathWinDriveLiftIss1383Suite pins — this suite brings the JS platform to parity (anti-cheat C11: every expected value is cited from java.nio semantics + the documented /C:/x model form).
  *
  * Red until ISS-1384 ports the lift into the scalajs FilePathPlatform.
  */
