...
/** Red tests for ISS-1092 (R0610): the Scala.js platform stores web-tree-sitter's `c.node.startIndex`/`c.node.endIndex` — UTF-16 code-unit indices into the JS source string — verbatim into
  ...
  *
  * Expected-value provenance: ssg-highlight is SSG-native (wraps tree-sitter via FFI, no original-src mapping — CLAUDE.md), so the canonical offset semantics are the JVM/Native FFI contract: the JVM
  * impl feeds `source.getBytes("UTF-8")` to `ts_parser_parse_string` and fills spans from `ts_node_start_byte`/`ts_node_end_byte` — i.e. `HighlightSpan.startByte`/`endByte` are UTF-8 byte offsets,
  * which is exactly what the renderer's `getBytes("UTF-8")` slicing consumes. The companion control suite `JvmUtf8OffsetsIss1092Suite` (src/test/scalajvm) asserts the identical
  * source/query/expectations on JVM and passes today, proving the expected values are the correct ones.
  *
  ...
  * }}}
  * So the correct span is [23,24) and the buggy JS span is [18,19); rendering the buggy span slices UTF-8 bytes 18..19 = the space after `const` (bytes 13..17 are `const`), wrapping `" "` instead of
  * `"1"`.
  *
...
final class JsUtf8OffsetsIss1092Suite extends munit.FunSuite {
  ...
  test("ISS-1092: JS platform spans carry UTF-8 byte offsets (JVM/Native contract), not UTF-16 code-unit indices") {
    assume(
      grammarAvailable,
      "ISS-1161: JS grammar loading unavailable in this environment (no wasm files under TREE_SITTER_WASM_DIR)"
    )
