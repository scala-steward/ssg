...
/** ISS-1095: Verifies that a native-library load failure produces a clear, actionable `IllegalStateException` that names the library and chains the original cause, instead of surfacing as a buried
  * `ExceptionInInitializerError`.
  *
  * The `loadLibrary` seam on `TreeSitterPlatformImpl` (JVM) accepts a custom loader function, allowing us to simulate a load failure without requiring the real native library to be absent.
  *
  * Proof-of-red: without the `loadLibrary` wrapper, calling the loader directly would propagate a raw `UnsatisfiedLinkError("boom")` whose message does NOT contain "tree_sitter_all" -- the assertion
  * on the message content would fail.
  */
