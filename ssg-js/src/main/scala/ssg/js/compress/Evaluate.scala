...
object Evaluate {
  ...
  /** Subset of regexps that is not going to cause regexp based DDOS. See: https://owasp.org/www-community/attacks/Regular_expression_Denial_of_Service_-_ReDoS
    *
    * Upstream (utils/index.js:244): const re_safe_regexp = /^[\\/|\0\s\w\^$.\[\]()]*$/;
    *
    * The char class includes NUL (U+0000). We embed a literal NUL character (written as a Scala unicode char-literal escape, see reSafeRegexp below) into the pattern string rather than using a regex
    * \0 escape, because re2 (Scala Native) rejects the \0 escape spelling in a char class but accepts a literal NUL byte. (History: ISS-201, ISS-1033/1135 audit probes, ISS-1138.)
    */
  private val reSafeRegexp = ("""^[\\/|""" + '\u0000' + """\s\w\^$.\[\]()]*$""").r

  /** Check if the regexp is safe for Terser to create without risking a RegExp DOS.
    *
    * Faithful to upstream regexp_is_safe (utils/index.js:247): export const regexp_is_safe = (source) => re_safe_regexp.test(source);
    */
  private[compress] def regexpIsSafe(source: String): Boolean =
