...
/** This file consists only of basic types used in multiple places. For types with javascript, create separate files by themselves.
  */
...
/** LaTeX argument type.
  ...
  *   - "url": An url string, in which "\" will be ignored if it precedes [#$%&~_^\{}]
  *   - "raw": A string, allowing single character, percent sign, and nested braces
  *   - "original": The same type as the environment that the function being parsed is in (e.g. used for the bodies of functions like \textcolor where the first argument is special and the second
