...
class Namespace[Value](
   ...
   *
   * Note: Do not use `if (namespace.get(...).isDefined)` to detect whether a macro
   * is defined, as the definition may be the empty string which evaluates
   * to `false` in JavaScript.  Use `if (namespace.get(...).isDefined)` or
   * `if (namespace.has(...))`.
   */
   ...
   * A value of Nullable.Null means to delete existing definitions.
   */
