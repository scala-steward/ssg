...
/** The main Settings object
  ...
  *   - displayMode: Whether the expression should be typeset as inline math (false, the default), meaning that the math starts in \textstyle and is placed in an inline-block); or as display math
  *     (true), meaning that the math starts in \displaystyle and is placed in a block with vertical margin.
  */
class Settings(
  ...
  /** Check whether to apply strict (LaTeX-adhering) behavior for unusual input (like `\\`). Unlike `nonstrict`, will not throw an error; instead, "error" translates to a return value of `true`, while
    * "ignore" translates to a return value of `false`. May still print a warning: "warn" prints a warning and returns `false`. This is for the second category of `errorCode`s listed in the README.
    */
  ...
  /** Check whether to test potentially dangerous input, and return `true` (trusted) or `false` (untrusted). The sole argument `context` should be an object with `command` field specifying the
    * relevant LaTeX command (as a string starting with `\`), and any other arguments, etc. If `context` has a `url` field, a `protocol` field will automatically get added by this function (changing
