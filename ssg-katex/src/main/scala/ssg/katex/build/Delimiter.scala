...
object Delimiter {
  ...
  /** Get the metrics for a given symbol and font, after transformation (i.e. after following replacement from symbols.js)
    */
  ...
  /** Puts a delimiter span in a given style, and adds appropriate height, depth, and maxFontSizes.
    */
  private def styleWrap(
    ...
    val span = BuildCommon.makeSpan(ArrayBuffer.from(classes) ++ newOptions.sizingClasses(options), ArrayBuffer(delim), Nullable(options))
  ...
  private def mathrmSize(
    ...
    BuildCommon.makeSymbol(value, "Size" + size + "-Regular", mode, Nullable(options))
  ...
  private def makeLargeDelim(
    ...
    val span  = styleWrap(
      BuildCommon.makeSpan(ArrayBuffer("delimsizing", "size" + size), ArrayBuffer[HtmlDomNode](inner), Nullable(options)),
      Style.TEXT,
      options,
      classes
    )
  ...
  /** Make a span from a font glyph with the given offset and in the given font. This is used in makeStackedDelim to make the stacking pieces for the delimiter.
    */
  private def makeGlyphSpan(
    ...
    val corner = BuildCommon.makeSpan(
      ...
      ArrayBuffer[HtmlDomNode](BuildCommon.makeSpan(ArrayBuffer.empty, ArrayBuffer[HtmlDomNode](BuildCommon.makeSymbol(symbol, font, mode))))
    )
  ...
  private def makeInner(
    ...
    val width: Double = {
      ...
      s4.flatMap(_.get(charCode)).map(_(4)).orElse(s1.flatMap(_.get(charCode)).map(_(4))).getOrElse(0.0)
    }
    val path    = new PathNode("inner", Nullable(SvgGeometry.innerPath(ch, Math.round(1000 * height).toInt)))
  ...
  private def makeStackedDelim(
    ...
    val repeatCount = Math.max(0, Math.ceil((heightTotal - minHeight) / (middleFactor * repeatHeightTotal)).toInt)
    ...
    styleWrap(
      BuildCommon.makeSpan(ArrayBuffer("delimsizing", "mult"), ArrayBuffer[HtmlDomNode](inner), Nullable(newOptions)),
      Style.TEXT,
      options,
      classes
    )
  ...
  def makeSqrtImage(
    ...
    val extraVinculum = Math.max(0.0, options.minRuleThickness - options.fontMetrics().sqrtRuleThickness)
    ...
    val (span, spanHeight, texHeight, advanceWidth): (SvgSpan, Double, Double, Double) = delim match {
      case _: DelimSmall =>
        ...
        val s  = sqrtSvg("sqrtMain", sh, viewBoxHeight, extraVinculum, options)
      ...
      case d: DelimLarge =>
        ...
        val s             = sqrtSvg("sqrtSize" + d.size, sh, viewBoxHeight, extraVinculum, options)
      ...
      case _: DelimStack =>
        ...
        val s             = sqrtSvg("sqrtTall", sh, viewBoxHeight, extraVinculum, options)
  ...
  def makeSizedDelim(
    ...
    if (
      stackLargeDelimiters.contains(delim) ||
      ...
      makeStackedDelim(delim, sizeToMaxHeight(size), false, options, mode, classes)
    } else {
      throw new ParseError("Illegal delimiter: '" + delim + "'")
    }
  }

  /** There are three different sequences of delimiter sizes that the delimiters follow depending on the kind of delimiter. This is used when creating custom sized delimiters to decide whether to
    * create a small, large, or stacked delimiter.
    *
    * In real TeX, these sequences aren't explicitly defined, but are instead defined inside the font metrics. Since there are only three sequences that are possible for the delimiters that TeX
    * defines, it is easier to just encode them explicitly here.
    */
  ...
  /** Get the font used in a delimiter based on what kind of delimiter it is. TODO(#963) Use more specific font family return type once that is introduced.
    */
  ...
  /** Make a delimiter for use with `\left` and `\right`, given a height and depth of an expression that the delimiters surround.
    */
  def makeLeftRightDelim(
    ...
    val maxDistFromAxis = Math.max(height - axisHeight, depth + axisHeight)
