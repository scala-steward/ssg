...
/**
 * Character metrics for a single glyph: depth, height, italic correction,
 * skew (kern from the character to the corresponding \skewchar), and width.
 */
...
/**
 * Font metrics for a given size. Contains all sigma/xi parameters plus
 * cssEmPerMu. The fields are stored in a map for dynamic access.
 */
...
object FontMetrics {
   ...
   *
   * Note: the `width` property may be undefined if fontMetricsData wasn't
  ...
  def getCharacterMetrics(
    ...
    val fontMap = metricMap.getOrElse(font,
      throw new ParseError(s"Font metrics not found for font: $font."))
