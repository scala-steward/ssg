...
/** This file provides support for Unicode range U+1D400 to U+1D7FF, Mathematical Alphanumeric Symbols.
  *
  * Function wideCharacterFont takes a wide character as input and returns the font information necessary to render it properly.
  */
object WideCharacter {

  /** Data below is from https://www.unicode.org/charts/PDF/U1D400.pdf That document sorts characters into groups by font type, say bold or italic.
    *
    * In the arrays below, each subarray consists three elements: * The CSS class of that group when in math mode. * The CSS class of that group when in text mode. * The font name, so that KaTeX can
    * get font metrics.
    */
