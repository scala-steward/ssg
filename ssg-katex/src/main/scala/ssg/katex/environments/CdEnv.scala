...
object CdEnv {
  ...
  private def cdArrow(
    ...
    funcName match {
      case "\\\\cdrightarrow" | "\\\\cdleftarrow" =>
        parser.callFunction(
          funcName, Array(labels(0)), Array(Nullable(labels(1)))
        )
      case "\\uparrow" | "\\downarrow" =>
        val leftLabel = parser.callFunction(
          "\\\\cdleft", Array(labels(0)), Array.empty
        )
        ...
        val rightLabel = parser.callFunction(
          "\\\\cdright", Array(labels(1)), Array.empty
        )
  ...
  def parseCD(parser: Parser): ParseNodeArray = {
          ...
          if ("=|.".contains(arrowChar)) {
            ...
            while (labelNum < 2) {
                ...
                if (isLabelEnd(rowNodes(k), arrowChar)) {
                  ...
                  if (isStartOfArrow(rowNodes(k))) {
                    throw new ParseError("Missing a " + arrowChar +
                      " character to complete a CD arrow.", Nullable(rowNodes(k)))
                  }
              ...
              if (inLabel) {
                ...
                throw new ParseError("Missing a " + arrowChar +
                  " character to complete a CD arrow.", Nullable(rowNodes(j)))
              }
              labelNum += 1
            }
          } else {
    ...
    val cols = Array.fill(numCols)(AlignSpec.Align(
      align = "c",
  ...
  def register(): Unit = {
    FunctionDef.defineFunction(FunctionDefSpec(
      ...
      mathmlBuilder = Nullable((group, options) => {
        ...
        var label = new MathNode("mrow",
          ArrayBuffer(BuildMathML.buildGroup(g.label, opts)))
    ...
    FunctionDef.defineFunction(FunctionDefSpec(
      ...
      mathmlBuilder = Nullable((group, options) => {
        ...
        val opts = options.asInstanceOf[Options]
        new MathNode("mrow",
          ArrayBuffer(BuildMathML.buildGroup(g.fragment, opts)))
      })
    ))
  }
}
