  ...
  def register(): Unit = {
    ...
    FunctionDef.defineFunction(FunctionDefSpec(
      ...
      htmlBuilder = Nullable((group, options) => {
        ...
        val upperGroup = BuildCommon.wrapFragment(
          BuildHTML.buildGroup(Nullable(g.body), newOptions, Nullable(opts)), opts)
        ...
        if (g.below.isDefined) {
          ...
          val lg = BuildCommon.wrapFragment(
            BuildHTML.buildGroup(g.below, newOptions, Nullable(opts)), opts)
      ...
      mathmlBuilder = Nullable((group, options) => {
        ...
        arrowNode.setAttribute(
          "minsize", if (g.label.charAt(0) == 'x') "1.75em" else "3.0em"
        )
        ...
        if (g.body != null) {
          ...
          if (g.below.isDefined) {
            ...
            node = new MathNode(
              "munderover", ArrayBuffer(arrowNode, lowerNode, upperNode)
            )
          } else {
