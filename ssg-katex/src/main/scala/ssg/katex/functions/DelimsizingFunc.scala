...
object DelimsizingFunc {
  ...
  final private case class DelimInfo(mclass: String, size: Int)
  ...
  private def checkDelimiter(
    ...
    if (symDelim.isDefined && delimiters.contains(symDelim.get.text)) {
      ...
      throw new ParseError(s"Invalid delimiter '${symDelim.get.text}' after '${context.funcName}'", Nullable(delim))
    } else {
      ...
      FunctionDefSpec(
        ...
        htmlBuilder = Nullable((group, options) =>
          boundary {
            ...
            Delimiter.makeSizedDelim(g.delim, g.size, opts, g.mode, Array(g.mclass))
          }
        ),
      ...
      FunctionDefSpec(
        ...
        handler = Nullable { (context, args, optArgs) =>
          ...
          val color: Nullable[String] = colorMacro.flatMap {
            ...
            case _                            => throw new ParseError("\\current@color set to non-string in \\right")
          }
    ...
    FunctionDef.defineFunction(
      FunctionDefSpec(
        ...
        handler = Nullable { (context, args, optArgs) =>
          ...
          val right = ParseNode.assertNodeType(parser.parseFunction(), "leftright-right").asInstanceOf[ParseNodeLeftrightRight]
          ParseNodeLeftright(
            mode = parser.mode,
            body = body,
            left = delim.text,
        ...
        htmlBuilder = Nullable((group, options) =>
          boundary {
            ...
            val inner = BuildHTML.buildExpression(g.body, opts, isRealGroup = true, surrounding = (Nullable("mopen"), Nullable("mclose")))
            ...
            val leftDelim: HtmlDomNode = if (g.left == ".") {
              ...
              // delimiter.
              Delimiter.makeLeftRightDelim(g.left, innerHeight, innerDepth, opts, g.mode, Array("mopen"))
            }
            ...
            val rightDelim: HtmlDomNode = if (g.right == ".") {
              ...
              Delimiter.makeLeftRightDelim(g.right, innerHeight, innerDepth, colorOptions, g.mode, Array("mclose"))
            }
        ...
        mathmlBuilder = Nullable { (group, options) =>
          ...
          if (g.left != ".") {
            val leftNode = new MathNode("mo", ArrayBuffer(BuildMathML.makeText(g.left, g.mode)))
            leftNode.setAttribute("fence", "true")
            inner.prepend(leftNode)
          }

          if (g.right != ".") {
            val rightNode = new MathNode("mo", ArrayBuffer(BuildMathML.makeText(g.right, g.mode)))
        ...
        htmlBuilder = Nullable((group, options) =>
          boundary {
            ...
            val middleDelim: DomSpan = if (g.delim == ".") {
              ...
              val d = Delimiter.makeSizedDelim(g.delim, 1, opts, g.mode, Array.empty)
