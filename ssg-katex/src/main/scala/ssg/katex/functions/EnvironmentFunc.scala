...
import ssg.katex.environments.{EnvironmentDef, EnvContext}
    ...
    FunctionDef.defineFunction(FunctionDefSpec(
      ...
      handler = Nullable((context, args, optArgs) => {
        ...
        while (i < og.body.length) {
          envName += ParseNode.assertNodeType(Nullable(og.body(i)), "textord")
            .asInstanceOf[ParseNodeTextord].text
          i += 1
        }
        ...
        if (context.funcName == "\\begin") {
          ...
          if (!EnvironmentDef._environments.contains(envName)) {
            throw new ParseError(
              "No such environment: " + envName, Nullable(nameGroup))
          }
          ...
          val end = ParseNode.assertNodeType(parser.parseFunction(), "environment")
            .asInstanceOf[ParseNodeEnvironment]
          if (end.name != envName) {
            throw new ParseError(
