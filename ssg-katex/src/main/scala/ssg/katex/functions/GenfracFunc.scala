...
object GenfracFunc {
  ...
  private val htmlBuilder: HtmlBuilder = (group, options) => {
    ...
    if (rule.isEmpty) {
      ...
      frac = BuildCommon.makeVList(VListParam.IndividualShift(
        children = Array(
    ...
    val leftDelim: HtmlDomNode =
      ...
      else Delimiter.makeCustomSizedDelim(
        g.leftDelim.get, delimSize, true,
        opts.havingStyle(style), g.mode, Array("mopen"))
    ...
    val rightDelim: HtmlDomNode =
      if (g.continued) BuildCommon.makeSpan(ArrayBuffer.empty) // zero width for \cfrac
        ...
        g.rightDelim.get, delimSize, true,
        opts.havingStyle(style), g.mode, Array("mclose"))
  ...
  def register(): Unit = {
    FunctionDef.defineFunction(FunctionDefSpec(
      ...
      handler = Nullable((context, args, optArgs) => {
        ...
        wrapWithStyle(ParseNodeGenfrac(
          mode = parser.mode,
          numer = numer,
          denom = denom,
          continued = continued,
    ...
    FunctionDef.defineFunction(FunctionDefSpec(
      ...
      handler = Nullable((context, args, optArgs) => {
        ...
        if (styl.nodeType == "ordgroup") {
          ...
          if (og.body.nonEmpty) {
            val textOrd = ParseNode.assertNodeType(Nullable(og.body(0)), "textord")
    ...
    FunctionDef.defineFunction(FunctionDefSpec(
      ...
      handler = Nullable((context, args, optArgs) => {
        ...
        ParseNodeInfix(
          ...
          size = Nullable(ParseNode.assertNodeType(Nullable(args(0)), "size")
            .asInstanceOf[ParseNodeSize].value),
          token = context.token
        )
      })
    ))
    ...
    FunctionDef.defineFunction(FunctionDefSpec(
      ...
      handler = Nullable((context, args, optArgs) => {
        val parser = context.parser.asInstanceOf[Parser]
        val numer = args(0)
        val barSize = ParseNode.assertNodeType(Nullable(args(1)), "infix")
          .asInstanceOf[ParseNodeInfix].size

        if (barSize.isEmpty) {
          throw new Error(
            s"\\\\abovefrac expected size, but got ${barSize.toString}")
        }
