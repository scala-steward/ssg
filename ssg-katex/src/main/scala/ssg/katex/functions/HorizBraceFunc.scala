...
object HorizBraceFunc {
  ...
  val htmlBuilder: HtmlBuilder = (grp, options) => {
    ...
    val body = BuildHTML.buildGroup(
      Nullable(group.base), opts.havingBaseStyle(Style.DISPLAY))
    ...
    if (group.isOver) {
      ...
      vlist.children(0).asInstanceOf[DomSpan].children(0).asInstanceOf[DomSpan]
        .children(1).asInstanceOf[DomSpan].classes += "svg-align"
    } else {
      vlist = BuildCommon.makeVList(VListParam.Positioned(
        positionType = "bottom",
      ...
      // TODO(ts): Replace this with passing "svg-align" into makeVList.
      vlist.children(0).asInstanceOf[DomSpan].children(0).asInstanceOf[DomSpan]
        .children(0).asInstanceOf[DomSpan].classes += "svg-align"
    }

    if (supSubGroup.isDefined) {
      // To write the supsub, wrap the first vlist in another vlist:
      val vSpan = BuildCommon.makeSpan(
        ArrayBuffer("minner", if (group.isOver) "mover" else "munder"),
        ArrayBuffer(vlist), Nullable(opts))
