      ...
      FunctionDefSpec(
        ...
        htmlBuilder = Nullable { (group, options) =>
          ...
          if (g.alignment == "clap") {
            ...
            inner = BuildCommon.makeSpan(ArrayBuffer.empty, ArrayBuffer(BuildHTML.buildGroup(Nullable(g.body), opts)))
            // wrap, since CSS will center a .clap > .inner > span
            inner = BuildCommon.makeSpan(ArrayBuffer("inner"), ArrayBuffer(inner), Nullable(opts))
          } else {
            inner = BuildCommon.makeSpan(ArrayBuffer("inner"), ArrayBuffer(BuildHTML.buildGroup(Nullable(g.body), opts)))
          }
          val fix  = BuildCommon.makeSpan(ArrayBuffer("fix"), ArrayBuffer.empty[HtmlDomNode])
          var node = BuildCommon.makeSpan(ArrayBuffer(g.alignment), ArrayBuffer(inner, fix), Nullable(opts))
