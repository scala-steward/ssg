...
object OpFunc {
  ...
  val htmlBuilder: HtmlBuilder = (grp, options) => {
    ...
    if (group.symbol) {
      ...
      base = BuildCommon.makeSymbol(
        group.name.getOrElse(""), fontName, Mode.Math, Nullable(opts),
        ArrayBuffer("mop", "op-symbol", if (large) "large-op" else "small-op"))

      if (stash.nonEmpty) {
        // We're in \oiint or \oiiint. Overlay the oval.
        val oval = BuildCommon.staticSvg(stash + "Size" +
          (if (large) "2" else "1"), opts)
        base = BuildCommon.makeVList(VListParam.IndividualShift(
          children = Array(
    ...
    var baseShift = 0.0
    var slant = 0.0
    if ((base.isInstanceOf[SymbolNode] ||
    ...
    if (hasLimits) {
      ...
      if (baseShift != 0) {
        ...
        htmlBase.style = htmlBase.style
          .copy(position = Nullable("relative"), top = Nullable(Units.makeEm(baseShift)))
      }
      base
    }
  }
  ...
  private val mathmlBuilder: MathMLBuilder = (group, options) => {
    ...
    if (g.symbol) {
      ...
      node = new MathNode(
        "mo", ArrayBuffer(BuildMathML.makeText(g.name.getOrElse(""), g.mode)))
      ...
      node = new MathNode(
        "mo", BuildMathML.buildExpression(g.body.get, opts))
    } else {
      // This is a text operator. Add all the characters from the
      // operator's name.
      node = new MathNode(
        "mi", ArrayBuffer(new TextNode(g.name.getOrElse("").substring(1))))
