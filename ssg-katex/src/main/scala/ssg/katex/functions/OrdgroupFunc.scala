    ...
    FunctionDef.defineFunctionBuilders(
      ...
      htmlBuilder = Nullable { (group, options) =>
        ...
        if (g.semisimple.getOrElse(false)) {
          BuildCommon.makeFragment(BuildHTML.buildExpression(g.body, opts, isRealGroup = false))
        } else {
          BuildCommon.makeSpan(ArrayBuffer("mord"), BuildHTML.buildExpression(g.body, opts, isRealGroup = true), Nullable(opts))
        }
      },
