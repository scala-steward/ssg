  ...
  def register(): Unit = {
    FunctionDef.defineFunction(FunctionDefSpec(
      ...
      htmlBuilder = Nullable((group, options) => {
        ...
        val innerGroup = BuildHTML.buildGroup(Nullable(g.body),
          opts.havingCrampedStyle())
        ...
        val vlist = BuildCommon.makeVList(VListParam.FirstBaseline(
          children = Array(
      ...
      mathmlBuilder = Nullable((group, options) => {
        val g = group.asInstanceOf[ParseNodeOverline]
        val opts = options.asInstanceOf[Options]
        val operator = new MathNode(
          "mo", ArrayBuffer(new TextNode("‾")))
        operator.setAttribute("stretchy", "true")

        val node = new MathNode(
          "mover",
          ArrayBuffer(BuildMathML.buildGroup(g.body, opts), operator))
