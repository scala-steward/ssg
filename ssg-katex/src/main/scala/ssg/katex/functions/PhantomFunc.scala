    ...
    FunctionDef.defineFunction(
      FunctionDefSpec(
        ...
        htmlBuilder = Nullable { (group, options) =>
          ...
          val opts  = options.asInstanceOf[Options]
          val inner = BuildCommon.makeSpan(ArrayBuffer("inner"), ArrayBuffer(BuildHTML.buildGroup(Nullable(g.body), opts.withPhantom())))
          val fix   = BuildCommon.makeSpan(ArrayBuffer("fix"), ArrayBuffer.empty[HtmlDomNode])
          BuildCommon.makeSpan(ArrayBuffer("mord", "rlap"), ArrayBuffer(inner, fix), Nullable(opts))
        },
        mathmlBuilder = Nullable { (group, options) =>
          val g       = group.asInstanceOf[ParseNodeVphantom]
          val opts    = options.asInstanceOf[Options]
