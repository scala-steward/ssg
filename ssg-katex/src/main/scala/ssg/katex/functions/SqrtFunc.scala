    ...
    FunctionDef.defineFunction(FunctionDefSpec(
      ...
      htmlBuilder = Nullable((group, options) => {
        ...
        inner.style = inner.style
          .copy(paddingLeft = Nullable(Units.makeEm(advanceWidth)))

        // Overlay the image and the argument.
        val body = BuildCommon.makeVList(VListParam.FirstBaseline(
          children = Array(
        ...
        if (g.index.isEmpty) {
          ...
          BuildCommon.makeSpan(ArrayBuffer("mord", "sqrt"),
            ArrayBuffer(rootVListWrap, body), Nullable(opts))
        }
      }),
      mathmlBuilder = Nullable((group, options) => {
        ...
        if (g.index.isDefined) {
          ...
          new MathNode(
            "msqrt", ArrayBuffer(BuildMathML.buildGroup(g.body, opts)))
        }
      })
    ))
  }
