    ...
    FunctionDef.defineFunctionBuilders(
      ...
      htmlBuilder = Nullable((group, options) => boundary {
        ...
        if (subm.isDefined) {
          ...
          val isOiint =
            g.base.isDefined && g.base.get.nodeType == "op" &&
              g.base.get.asInstanceOf[ParseNodeOp].name.exists(n =>
                n == "\\oiint" || n == "\\oiiint")
        ...
        if (supm.isDefined && subm.isDefined) {
          supShift = Math.max(Math.max(
            supShift, minSupShift), supm.get.depth + 0.25 * metrics.xHeight)
          ...
          val vlistElem = Array(
            VListElemAndShift(
              elem = subm.get, shift = subShift,
              marginRight = Nullable(marginRight),
              marginLeft = marginLeft),
            VListElemAndShift(
              elem = supm.get, shift = -supShift,
              marginRight = Nullable(marginRight))
          )
          ...
          // Rule 18b
          subShift = Math.max(Math.max(
            subShift, metrics.sub1),
            subm.get.height - 0.8 * metrics.xHeight)

          val vlistElem: Array[VListChild] = Array(
            VListElem(elem = subm.get,
              marginLeft = marginLeft,
              marginRight = Nullable(marginRight))
          )
          ...
          // Rule 18c, d
          supShift = Math.max(Math.max(supShift, minSupShift),
            supm.get.depth + 0.25 * metrics.xHeight)

          supsub = BuildCommon.makeVList(VListParam.Positioned(
            positionType = "shift",
            positionData = -supShift,
            children = Array(
              VListElem(elem = supm.get,
                marginRight = Nullable(marginRight))
            )
          ), opts)
        } else {
        ...
        BuildCommon.makeSpan(ArrayBuffer(mclass),
          ArrayBuffer(base, BuildCommon.makeSpan(ArrayBuffer("msupsub"), ArrayBuffer(supsub))),
          Nullable(opts))
      }),
