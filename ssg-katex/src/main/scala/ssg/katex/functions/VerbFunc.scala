...
object VerbFunc {
  ...
  /** Converts verb group into body string.
    *
    * \verb* replaces each space with an open box ␣ \verb replaces each space with a no-break space \xA0
    */
      ...
      FunctionDefSpec(
        ...
        htmlBuilder = Nullable { (group, options) =>
          ...
          while (i < text.length) {
            ...
            body += BuildCommon.makeSymbol(c, "Typewriter-Regular", g.mode, Nullable(newOptions), ArrayBuffer("mord", "texttt"))
            i += 1
          }
