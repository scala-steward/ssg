...
class MacroExpander(input: String, val settings: Settings, var mode: Mode)
    extends MacroContextInterface {
  ...
  def consumeArg(delims: Nullable[Array[String]] = Nullable.Null): MacroArg = {
    ...
    while (!loopDone) {
      ...
      if (isDelimited) {
        ...
        if ((depth == 0 || (depth == 1 && delimArr(matchCount) == "{")) &&
    ...
    if (delimiters.isDefined) {
      ...
      while (i < delims.length) {
        ...
        if (delims(i) != tok.text) {
          throw new ParseError(
            "Use of the macro doesn't match its definition", Nullable(tok))
        }
        i += 1
      }
    }
  ...
  def expandOnce(expandableOnly: Boolean = false): Int | Boolean = boundary {
    ...
    if (expansion.isEmpty || (expandableOnly && expansion.get.unexpandable)) {
      if (expandableOnly && expansion.isEmpty &&
