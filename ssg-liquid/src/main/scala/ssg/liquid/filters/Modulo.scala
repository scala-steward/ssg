 ...
 *
 * Ported from: liqp/src/main/java/liqp/filters/Modulo.java
 * Original: Copyright (c) 2012 Bart Kiers
 * Original license: MIT
 *
 * Covenant: full-port
 * Covenant-java-reference: src/main/java/liqp/filters/Modulo.java
...
class Modulo extends Filter {
  ...
  override def apply(value: DataView, context: TemplateContext, params: Array[DataView]): DataView = {
    ...
    if (canBeInteger(v) && canBeInteger(rhsObj)) {
      ...
      DataView.from(LValue.asFormattedNumber(first.remainder(second)))
    }
  }
}
