...
/** RFC 3492 Punycode encoding — pure Scala, no JVM-only dependencies.
  *
  * Provides `encode` (code-point array to Punycode string) and `toAscii`
  * (internationalized hostname to ASCII-Compatible Encoding per RFC 3490).
  */
private[liquid] object Punycode {
  ...
  /** Convert an internationalized hostname to ASCII-Compatible Encoding.
    *
    * Splits the host on '.', Punycode-encodes each non-ASCII label with the
    * "xn--" ACE prefix, and reassembles. Pure-ASCII labels pass through unchanged.
    */
