...
final class FilterArraySuite extends munit.FunSuite {
  ...
  test("first: returns the first element of an array") {
    ...
    values.add("foo")
    values.add("bar")
    vars.put("values", TestHelper.dv(values))
  ...
  test("sort: sorts an array of numbers") {
    ...
    vars.put("numbers", TestHelper.dv(numbers))
  ...
  test("sort_natural: case-insensitive sort") {
    ...
    words.add("apple")
    words.add("Cherry")
    vars.put("words", TestHelper.dv(words))
  ...
  test("map: extracts a property from array of maps") {
    ...
    val p1       = new HashMap[String, DataView]()
    p1.put("title", TestHelper.dv("Shoes"))
    val p2 = new HashMap[String, DataView]()
    p2.put("title", TestHelper.dv("Shirt"))
    val p3 = new HashMap[String, DataView]()
    p3.put("title", TestHelper.dv("Pants"))
    products.add(p1)
    products.add(p2)
    products.add(p3)
    vars.put("products", TestHelper.dv(products))
  ...
  test("where: filters array of maps by property") {
    val vars  = new HashMap[String, DataView]()
    val items = new ArrayList[Any]()
    val item1 = new HashMap[String, DataView]()
    item1.put("color", TestHelper.dv("red"))
    item1.put("name", TestHelper.dv("apple"))
    val item2 = new HashMap[String, DataView]()
    item2.put("color", TestHelper.dv("blue"))
    item2.put("name", TestHelper.dv("berry"))
    val item3 = new HashMap[String, DataView]()
    item3.put("color", TestHelper.dv("red"))
    item3.put("name", TestHelper.dv("cherry"))
    items.add(item1)
    items.add(item2)
    items.add(item3)
    vars.put("items", TestHelper.dv(items))
