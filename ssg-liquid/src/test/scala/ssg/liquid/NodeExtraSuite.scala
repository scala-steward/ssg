...
final class NodeExtraSuite extends munit.FunSuite {
  ...
  test("block node: custom tag with block") {
    val parser = new TemplateParser.Builder().withBlock(new blocks.Block("testtag") {
  ...
  test("output node: allowed keywords as variables") {
    val keywords = Array(
      "capture", "endcapture", "comment", "endcomment", "raw", "endraw",
      "if", "elsif", "endif", "unless", "endunless", "else", "contains",
      "case", "endcase", "when", "cycle", "for", "endfor", "in", "and", "or",
      "tablerow", "endtablerow", "assign", "include", "with", "end",
      "break", "continue", "offset", "reversed"
    )
  ...
  test("lookup: size of hash") {
    val vars = TestHelper.mapOf(
      "hash" -> TestHelper.mapOf(
        "a" -> java.lang.Integer.valueOf(1), "b" -> java.lang.Integer.valueOf(2),
        "c" -> java.lang.Integer.valueOf(3), "d" -> java.lang.Integer.valueOf(4)
      )
    )
  ...
  test("lookup: collection access by index") {
    val vars = Collections.singletonMap[String, Any](
      "data", new LinkedHashSet[Any](Arrays.asList("hello", "world"))
    )
  ...
  test("lookup: negative offset") {
    // LinkedHashSet
    assertEquals(
      Template.parse("Hello {{data[-2]}}").render(
  ...
  test("lookup: out of size returns default") {
    assertEquals(
      Template.parse("Hello {{data[99] | default: 'default'}}").render(
        Collections.singletonMap[String, Any]("data", new LinkedHashSet[Any](Arrays.asList("hello", "world")))
      ),
      "Hello default"
    )
  ...
  test("comparing expression: bug 286 case 1") {
    val parser = new TemplateParser.Builder()
      .withFlavor(Flavor.JEKYLL)
      .build()
    val res = parser.parse("{%- assign value = 5.0 | round: 1 -%} {%- if value >= 4.0 and value <= 4.2 -%} Très bien {%- elsif value >= 4.3 and value <= 4.7 -%} Superbe {%- elsif value >= 4.8 and value <= 4.9 -%} Fabuleux {%- elsif value != 5.0 -%} Exceptionnel {% endif %}").render()
  ...
  test("gt node: bug 267 expression in output as Liquid".fail) {
    try {
      new TemplateParser.Builder().withFlavor(Flavor.LIQUID).build()
        .parse("{{ 98 > 97 }}").render()
  ...
  test("gt node: filter compare") {
    val result = new TemplateParser.Builder().withFlavor(Flavor.JEKYLL).build()
