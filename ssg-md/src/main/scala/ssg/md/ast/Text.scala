 ...
 *
 * Ported from: flexmark/src/main/java/com/vladsch/flexmark/ast/Text.java
 * Original: Copyright (c) 2016-2023 Vladimir Schneider
 * Original license: BSD-2-Clause
 *
 * Covenant: full-port
 * Covenant-java-reference: flexmark/src/main/java/com/vladsch/flexmark/ast/Text.java
...
final class Text extends Node, TextContainer {
  ...
  def this(chars: String, baseSeq: BasedSequence) = {
    this()
    this.chars = PrefixedSubSequence.prefixOf(chars, baseSeq)
  }
  ...
  override def astExtra(out: StringBuilder): Unit = {
    astExtraChars(out)
    if (chars.isInstanceOf[PrefixedSubSequence]) {
      Node.astChars(out, chars, "text")
    }
  }
