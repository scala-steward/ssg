...
object AbbreviationExtension {
  ...
  val ABBREVIATIONS: DataKey[AbbreviationRepository] = new DataKey[AbbreviationRepository]("ABBREVIATIONS", new AbbreviationRepository(Nullable.empty), (options: DataHolder) => new AbbreviationRepository(Nullable(options)))
