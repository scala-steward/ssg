 ...
 *
 * Covenant: full-port
 * Covenant-java-reference: flexmark-ext-attributes/src/main/java/com/vladsch/flexmark/ext/attributes/AttributesExtension.java
...
/** Extension for attributes
  ...
  */
class AttributesExtension private () extends Parser.ParserExtension, RendererExtension, HtmlRenderer.HtmlRendererExtension, Formatter.FormatterExtension {

  override def parserOptions(options: MutableDataHolder): Unit =
    if (
      options
        .contains(AttributesExtension.FENCED_CODE_INFO_ATTRIBUTES) && AttributesExtension.FENCED_CODE_INFO_ATTRIBUTES.get(options) && !options.contains(AttributesExtension.FENCED_CODE_ADD_ATTRIBUTES)
    ) {
...
object AttributesExtension {
  ...
  val NODE_ATTRIBUTES: DataKey[NodeAttributeRepository] = new DataKey[NodeAttributeRepository](
    "NODE_ATTRIBUTES",
    new NodeAttributeRepository(null),
    (options: ssg.md.util.data.DataHolder) => new NodeAttributeRepository(options)
  ) // @nowarn - Java interop
