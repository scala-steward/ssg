...
class FootnoteExtension private ()
  ...
  override def transferReferences(document: MutableDataHolder, included: DataHolder): Boolean = {
    if (document.contains(FootnoteExtension.FOOTNOTES) && included.contains(FootnoteExtension.FOOTNOTES)) {
      Parser.transferReferences(FootnoteExtension.FOOTNOTES.get(document), FootnoteExtension.FOOTNOTES.get(included), FootnoteExtension.FOOTNOTES_KEEP.get(document) == KeepType.FIRST)
    } else {
      false
    }
  }
