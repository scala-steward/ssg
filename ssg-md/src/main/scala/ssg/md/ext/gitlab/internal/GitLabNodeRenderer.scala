  ...
  private def renderVideoImage(srcNode: Node, url: String, altText: String, attributes: Attributes, html: HtmlWriter): Boolean = {
    ...
    if (pos != -1) {
      ...
      if (gitLabOptions.videoImageExtensionSet.contains(extension)) {
        html.attr(Attribute.CLASS_ATTR, gitLabOptions.videoImageClass).attr(attributes).withAttr().tagLineIndent("div", () => {
          html.srcPos(srcNode.chars)
            .attr("src", url)
            .attr("width", "400")
            .attr("controls", "true")
            .withAttr(GitLabNodeRenderer.VIDEO)
            .tag("video")
            .tag("/video")
            .line()
