...
class MacrosExtension private () extends Parser.ParserExtension
  ...
  override def transferReferences(document: MutableDataHolder, included: DataHolder): Boolean = {
    ...
    if (included.contains(MacrosExtension.MACRO_DEFINITIONS)) {
      Parser.transferReferences(MacrosExtension.MACRO_DEFINITIONS.get(document), MacrosExtension.MACRO_DEFINITIONS.get(included), MacrosExtension.MACRO_DEFINITIONS_KEEP.get(document) == KeepType.FIRST)
    } else {
      false
    }
  }
...
object MacrosExtension {
  ...
  val MACRO_DEFINITIONS: DataKey[MacroDefinitionRepository] = new DataKey[MacroDefinitionRepository]("MACRO_DEFINITIONS", new MacroDefinitionRepository(null), (options: DataHolder) => new MacroDefinitionRepository(options))
