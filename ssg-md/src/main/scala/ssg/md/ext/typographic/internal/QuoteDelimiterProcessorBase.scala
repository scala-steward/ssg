...
class QuoteDelimiterProcessorBase(
  ...
  protected def canClose(closer: DelimiterRun, minLength: Int): Boolean = {
    if (closer.canClose) {
      ...
      if (closer.next != null && closerChars.isContinuationOf(closer.next.node.chars) || closerChars.endOffset >= closerChars.getBaseSequence.length() || isAllowed(closerChars.getBaseSequence, closerChars.endOffset + minLength - 1)) {
        true
      } else {
        false
      }
  ...
  protected def canOpen(opener: DelimiterRun, minLength: Int): Boolean = {
    if (opener.canOpen) {
      ...
      if (opener.previous != null && opener.previous.node.chars.isContinuationOf(openerChars) || openerChars.startOffset == 0 || isAllowed(openerChars.getBaseSequence, openerChars.startOffset - minLength)) {
        true
      } else {
        false
      }
