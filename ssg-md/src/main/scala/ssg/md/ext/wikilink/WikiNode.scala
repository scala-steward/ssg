...
class WikiNode(val linkIsFirst: Boolean) extends Node with DoNotDecorate with TextContainer with LinkRefDerived {
  ...
  override def collectText(out: ISequenceBuilder[? <: ISequenceBuilder[?, BasedSequence], BasedSequence], flags: Int, nodeVisitor: NodeVisitor): Boolean = {
    ...
    if (urlType != TextContainer.F_LINK_TEXT && urlType != TextContainer.F_LINK_PAGE_REF && urlType != TextContainer.F_LINK_ANCHOR && urlType != TextContainer.F_LINK_URL && urlType != TextContainer.F_LINK_NODE_TEXT) {
