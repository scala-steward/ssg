 ...
 *
 * Ported from: flexmark-ext-autolink/.../ComboAutolinkSpecTest.java
...
object ComboAutolinkSpecTest {
  ...
  val OPTIONS_MAP: java.util.Map[String, DataHolder] = {
    ...
    map.put("no-autolink", new MutableDataSet()
      .set(TestUtils.UNLOAD_EXTENSIONS, Collections.singletonList(classOf[AutolinkExtension]))
      .toImmutable)
    ...
    map.put("typographic-ext", new MutableDataSet()
      .set(Parser.EXTENSIONS, Arrays.asList(AutolinkExtension.create(), TypographicExtension.create()))
      .toImmutable)
    map
  }
}
