...
object ComboGfmTaskListSpecTest {
  ...
  val OPTIONS_MAP: java.util.Map[String, DataHolder] = {
    ...
    map.put("not-done", new MutableDataSet().set(TaskListExtension.ITEM_NOT_DONE_MARKER, "<span class=\"taskitem\">O</span>").toImmutable)
    map.put("no-ordered-items", new MutableDataSet().set(Parser.LISTS_NUMBERED_ITEM_MARKER_SUFFIXED, false).toImmutable)
    map.put("kramdown", new MutableDataSet()
      .setFrom(ParserEmulationProfile.KRAMDOWN)
      .set(Parser.EXTENSIONS, Collections.singleton(TaskListExtension.create()))
      .toImmutable)
    map.put("markdown", new MutableDataSet()
