...
final class MarkdownTableTest extends MarkdownTableTestBase {
  ...
  test("test_LeftAligned_BackspaceAfterEmpty") {
    ...
    val table  = getTable(
      source,
      formatOptions("", null).toMutable.set(TableFormatOptions.FORMAT_TABLE_LEFT_ALIGN_MARKER, DiscretionaryText.AS_IS)
    )
  ...
  test("test_Separator_CenteredBackspaceLastColonRemoveLeftColon") {
    ...
    val table  = getTable(
      source,
      formatOptions("", null).toMutable.set(TableFormatOptions.FORMAT_TABLE_LEFT_ALIGN_MARKER, DiscretionaryText.REMOVE)
    )
    assert(table.addTrackedOffset(TrackedOffset.track(pos, Nullable.empty[Character], false)))
    val out = new HtmlWriter(0, LineAppendable.F_FORMAT_ALL)
    table.appendTable(out)
  ...
  test("test_EmbeddedPipe0Options") {
    ...
    val table  = getTable(
      source,
      formatOptions("", null).toMutable.set(TablesExtension.FORMAT_TABLE_FILL_MISSING_COLUMNS, true).set(TableFormatOptions.FORMAT_TABLE_FILL_MISSING_MIN_COLUMN, Nullable(Integer.valueOf(0)))
    )
//        table.fillMissingColumns(Nullable(Integer.valueOf(0)))
  ...
  test("test_indentPrefix") {
    ...
    val table  = getTable(
      source,
      formatOptions("", null).toMutable.set(TablesExtension.FORMAT_TABLE_FILL_MISSING_COLUMNS, false).set(TablesExtension.FORMAT_TABLE_INDENT_PREFIX, "    ")
    )
    table.fillMissingColumns()
  ...
  test("forAllRows") {
    ...
    var count = 0
    table.forAllRows(
      new ssg.md.util.format.TableRowManipulator {
        override def apply(
