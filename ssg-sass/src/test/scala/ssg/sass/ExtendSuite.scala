...
final class ExtendSuite extends munit.FunSuite {
  ...
  test("@extend trims superselector without combinator (RC-3)") {
    // directives/extend/trims_super_selector_without_combinator.hrx
    // Both `a b` and `a > b` extend `%c`. Since both are original selectors
    // (from the document, not generated by @extend), dart-sass preserves both
    // — original selectors are never trimmed by `_trim`.
    val src =
      """a b {
        |  @extend %c;
        |}
    ...
    assert(
      css.contains("a b"),
      s"expected 'a b' in output, got:\n$css"
    )
    assert(
      css.contains("a > b"),
      s"expected 'a > b' in output (originals are never trimmed), got:\n$css"
    )
  }

  test("@extend trimming preserves non-placeholder originals") {
    // When extending a non-placeholder selector, the original should be preserved
    // even though extensions may also be structurally equal to other originals.
    val src =
      """.a { color: red; }
        |.b { @extend .a; }""".stripMargin
    val result = Compile.compileString(src)
    val css    = result.css
    assert(
      css.contains(".a, .b"),
  ...
  test("@extend trimming with identical selectors in different rules") {
    // When the same selector appears in both an extending rule and the extended
    // result, it should not be incorrectly marked as original for the wrong list.
    val src =
      """.a { color: red; }
        |.a { @extend .b !optional; }
        |.b { x: y; }""".stripMargin
    val result = Compile.compileString(src)
    val css    = result.css
    // Both .a and .b rules should appear in the output
    assert(
      css.contains(".a"),
      s"expected '.a' in output, got:\n$css"
    )
  }

  test("three-level extend loop produces correct selector ordering") {
    // non_conformant/extend-tests/181_test_three_level_extend_loop.hrx
    val src =
      """.foo {a: b; @extend .bar}
        |.bar {c: d; @extend .baz}
        |.baz {e: f; @extend .foo}""".stripMargin
    val result = Compile.compileString(src)
    val css    = result.css
    // Each rule should contain all three selectors
    assert(
      css.contains(".foo, .baz, .bar"),
      s"expected '.foo, .baz, .bar' for first rule, got:\n$css"
    )
    assert(
      css.contains(".bar, .foo, .baz"),
      s"expected '.bar, .foo, .baz' for second rule, got:\n$css"
    )
    assert(
      css.contains(".baz, .bar, .foo"),
      s"expected '.baz, .bar, .foo' for third rule, got:\n$css"
    )
  }
}
