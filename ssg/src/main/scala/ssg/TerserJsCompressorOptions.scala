...
/** Terser-backed [[ssg.minify.JsCompressorOptions]].
  *
  * Fields mirror the `terser_args` keys jekyll-minifier supports
  * (lib/jekyll-minifier.rb:799-841 `validate_compressor_args`) that ssg-js's
  * Terser engine can honor. Each maps onto ssg-js `MinifyOptions` via
  * [[toMinifyOptions]]:
  *
  *   - `compress` — enable the compress phase (`true` → default
  *     [[ssg.js.compress.CompressorOptions]], `false` → disabled), matching
  *     `::Terser.new(compress: …)`.
  *   - `mangle` — enable name mangling (`true` → default
  *     [[ssg.js.scope.ManglerOptions]], `false` → disabled).
  *   - `ecma` — ECMAScript output/compress version; flows to both
  *     `CompressorOptions.ecma` and `OutputOptions.ecma`.
  *   - `ie8` — IE8 compatibility; flows to both `CompressorOptions.ie8` and
  *     `ManglerOptions.ie8`.
  ...
  *   - `evaluate` — evaluate the `eval` (top-level eval mangling) terser_arg;
  *     flows to `ManglerOptions.eval`. Named `evaluate` to avoid shadowing the
  *     `eval` reserved word; carries jekyll-minifier's `eval` terser_arg.
  *
  * Defaults mirror ssg-js Terser's own defaults (compress + mangle on, ECMA 5,
  * ie8/safari10/toplevel/eval off) so `TerserJsCompressorOptions()` behaves like
  * the no-argument `::Terser.new()` path.
  */
final case class TerserJsCompressorOptions(
  ...
  /** Map these terser_args onto ssg-js Terser's [[ssg.js.MinifyOptions]].
    *
    * Mirrors `::Terser.new(terser_args)` (lib/jekyll-minifier.rb:384/435): a
    * boolean `compress`/`mangle` toggles the respective phase, while numeric/
    * boolean `ecma`/`ie8`/`safari10` and boolean `toplevel`/`eval` configure the
    * options objects. ssg-js follows upstream Terser's Boolean semantics
    * (Terser.scala:36-51): `true` selects the default options object, `false`
    * disables the phase.
    */
